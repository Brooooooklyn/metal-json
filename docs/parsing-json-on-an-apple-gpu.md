...
|----------------------|--------------------------|---------------------------|--------|
| rounds=1 (classifier mix)    | 4.507 ms (238 GB/s)      | 2.198 ms (489 GB/s)       | 2.05x  |
...
Then the token-mask kernel computes tokens per word, bit-exactly matching the CPU reference:
...
Carries between 64 KiB chunks flow through a two-level scan: the classifier and token-mask kernels emit one partial per chunk, and the two spine-scan kernels each run as a *single* 256-thread threadgroup that rewrites the partials in place as an exclusive prefix sum. No decoupled look-back scans anywhere — Apple GPUs give no forward-progress guarantee between threadgroups, so the design never lets one threadgroup spin-wait on another. The scatter kernel then writes tokens to their globally dense ranks: position and kind arrays, every slot written exactly once.

The scatter kernel sits in its own command buffer for a principled reason: its output buffers cannot exist until the CPU has read `token_total`. The cost is one extra sync round trip, priced at ~50–160 µs by spike C, accepted for exact-fit allocation. The same rule later added CB2b — the fourth command buffer the plan didn't have.

UTF-8 validation lives inside the classifier with exact first-error offsets (the same offset Rust's `str::from_utf8` reports; a test pins the equivalence). The first version walked every non-ASCII word byte-by-byte, and that walk was 75% of the classifier's time on twitter — where 30% of words contain non-ASCII bytes. The fix is a two-tier design: a register-only bitmask check *proves* the well-formed common case, and only inconclusive words fall to the scalar walk, which remains the single source of error offsets. The proof obligation is one-sided — fast-pass implies the walk accepts — so error behavior can't drift. That change cut the classifier's runtime by about 60%.
...
scatter + tape-offset kernels in own command buffers     ~50-160 us extra sync each     exact-fit allocations, never
